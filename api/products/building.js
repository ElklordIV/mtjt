...
async function fetchBuildingProducts() {
    try {
        // Fetch products specifically for the Building page
        const response = await fetch('/api/products/building');
        if (response.ok) {
...
function fetchProducts() {
    fetch('/getBuildingProducts') // Adjust the endpoint to your actual route
...
function displayProducts(products) {
    ...
    categories.forEach(category => {
        ...
        const categoryProducts = products.filter(product => product.category === category).slice(0, 3); // Limit to 3 items
